      ...
      <div className="md-dialog upd-dialog" onClick={e => e.stopPropagation()}>
        ...
        <div className="md-body upd-body">
          ...
          <div className="upd-install">
            Install with <code>npm i -g xshell-app@latest</code>, or download the binary from the release page.
          </div>
          {info.releaseNotes && (
            <div className="upd-notes">
              <div className="upd-notes-head">Release notes — v{info.latestVersion}</div>
              <pre className="upd-notes-body">{info.releaseNotes}</pre>
            </div>
          )}
        </div>
        <div className="upd-foot">
          ...
          {info.releaseUrl && (
            <button className="btn btn-primary" onClick={() => { invoke("open_url", { url: info.releaseUrl! }).catch(() => {}); onDismiss(); }}>
